...
const handleAddRow = (rowIndex) => {
    ...
    // Add a new row if the rowIndex matches the last row
    if (rowIndex === lstitem.length - 1) {
        setLstitem([
            ...lstitem,
            { itemName: "", grossWt: 0, lessWt: 0, netWt: 0, isOther: false },
        ]);
...
const handleCellKeyPress = (event) => {
    const { colDef, node, key } = event;

    // Check if the Enter key is pressed and if it's the last column
    if (key === "Enter") {
        const isLastColumn = colDef.field === firstGridColumnDefs[firstGridColumnDefs.length - 1].field;
        if (isLastColumn) {
            handleAddRow(node.rowIndex);
        }
    }
};
...
const calculateTotals = (items) => {
    const totals = items.reduce(
        (acc, row) => {
            acc.grossWt += row.grossWt || 0;
            acc.lessWt += row.lessWt || 0;
            acc.netWt += row.netWt || 0;
            return acc;
        },
        { grossWt: 0, lessWt: 0, netWt: 0 }
    );
...
--------------------------------------------------------------------------------
  const pinnedBottomRowDataFirstGrid = useMemo(() => {
    const totals = calculateTotals(lstitem);

    return [
        {
            itemName: "Totals",
            grossWt: totals.grossWt,
            lessWt: totals.lessWt,
            netWt: totals.netWt,
            editable: false,
        },
    ];
